...
Expected: FAIL —— 现行 `sanitizeLockEntries` 只在「磁盘不存在」时才登记 `invalidPaths`，`../evil.txt` 会被 `resolveWriteTarget` 判成 `null` 而**在 reader 里抛 `PATH_ESCAPE`**，测试要求的 `details.escapingPaths` 形状拿不到（错误码也不对）。这正是本任务要修的：净化必须在**规划之前**跑完并聚合，而不是靠 reader 逐条炸。

- [ ] **Step 3: 收紧净化实现**

把 `sanitizeLockEntries` 换成聚合式（与 `checkPackInjectable` 同类：一次跑完全部规则、按码点序出清单）：

```ts
/** 删除前逐条走 design §7.7 净化：任一违规即整包拒绝（含合法条目），防「半退场」 */
function sanitizeLockEntries(projectPath: string, lock: PackLock): void {
  const invalidPaths: string[] = []
  const escapingPaths: string[] = []
  for (const entry of lock.files) {
  ...
  if (invalidPaths.length > 0 || escapingPaths.length > 0) {
    throw new CleanError(
      'VALIDATION_ERROR',
      `pack.lock.json 有 ${String(invalidPaths.length + escapingPaths.length)} 条路径未通过净化，` +
        '整包拒绝退场（合法条目同样不删，半退场比不退场更难查）',
      {
...
- [ ] **Step 5: 交棒**：两支新用例的输出 + `sanitizeLockEntries` 的 diff，等放行。
