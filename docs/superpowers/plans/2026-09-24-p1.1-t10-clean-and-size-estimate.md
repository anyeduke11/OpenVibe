...
**Files:**
- Modify: `apps/cli/test/clean.test.ts`（追加两个 describe）
- Modify: `apps/cli/src/commands/clean.ts`（`sanitizeLockEntries` 收紧）

**Interfaces:**
- Consumes: Task 3 的 `cleanAction`、`apps/cli/test/helpers/sync-lock-holder.ts`（command 写 `sync-holder`，clean 只需「有人持锁」这一事实，无需新夹具）
- Produces: 净化失败的 `details.escapingPaths` 形状（Task 5 的 `violationLines` 渲染依赖它）
