...
2. `CLI-CLEAN-*` 实为 **17 支**（§7.10 a–j 十支 + 七支 sub-branch：`01b` 退场后 diff 报未注入、`04b` 无 lock 的 dry-run 零副作用、`05b` TTY 选「否」、`06b` 全 FOREIGN 保留 lock、`06c` `LOCK_INVALID`（↔ 规格 f2）、`11`/`12` `--json` 契约与命令注册），不得写成「规格要求的十支全绿」；
...
4. 未验证面：Web 显示无 DOM 断言（Step 8-4）、win32/linux 未见 CI、`clean` 的中断恢复（删一半失败）只有单测覆盖，未做真 SIGKILL 演练。
