...
describe('零副作用与前置（§7.10 d、e、f）', () => {
  ...
  it('CLI-CLEAN-06b: 无可删项（全部 ABSENT：用户自己删过注入文件）→ 照样收尾并删 lock，cleaned=true（FR-6.5 v1.4 / §7.10 e3）', async () => {
    ...
    const lockKey = PACK_LOCK_REL.split(/[\\/]/).join('/')
    expect(Object.keys(before).filter((k) => !(k in after))).toEqual([lockKey])
...
Run: `npx vitest run --project cli apps/cli/test/clean.test.ts`
Expected: PASS（`CLI-CLEAN-01..06c` 全绿；本轮实际落 `01,01b,02,03,04,04b,04c,05,05b,05c,05d,06,06b,06c` = **14 支**，`07..12` 属后续任务）
