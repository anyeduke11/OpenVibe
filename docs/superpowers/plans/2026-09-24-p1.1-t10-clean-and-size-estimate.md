...
`apps/cli/test/clean.test.ts` 里凡依赖 POSIX 语义的断言（权限位、符号链接）本轮**没有**新增，故 win32 应无新增跳过；仍需在 DEV_LOG 写清「本地仅 darwin 验证，linux/win32 以 CI 为准」，并按 [[openvibe-ci-reading-verdicts]] 的口径分平台报数。
